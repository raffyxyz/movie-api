...
export const scrapeDropdown = async ({ type = 2 }) => {
    let list = [];
    try {
        const searchGenres = await axios.get(
            `${BASE_URL + home_path}`
        );

        const $ = cheerio.load(searchGenres.data);

        $(`.header_menu-list > .nav-item:nth-child(${type}) > .header_menu-sub > .sub-menu > li`).each((i, el) => {
            list.push({
                name: $(el).find('a').attr('title'),
                src: $(el).find('a').attr('href').split('/')[2],
            });
        })
